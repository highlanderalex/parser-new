<?php

	define('BASE_SITE','https://www.carid.com');
	define('MAIN_PAGE','suspension-systems.html');
	define('LOG_FILE','storage/errors/log.txt');
	define('TMP_DIR','tmp/');
	define('PATH','data/');
	define('USE_PROXY',0);
	define('EMAIL','highlanderalex@rambler.ru');
	define('FROM_EMAIL','highlanderalex.hmax@gmail.com');
	define('PAGES', 5);
	define ('HOST','localhost');
	define ('DBNAME','carid');
	define ('DBDRIVER','mysql');
	define ('LOGIN','root');
	define ('PASS','');
	define('SUCCESS_SCRIPT', 'Script finished: SUCCESS');
	define('FAIL_SCRIPT', 'Script finished: FAIL');
	define('FAIL_URL', 'HTTP REQUEST FAILED');
	define('FAIL_DATA', 'DATA NOT FOUND');